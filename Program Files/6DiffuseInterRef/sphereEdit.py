...
class Sphere:
    ...
    def __init__(self, r, c, col, clas = None, dif = 12):
        ...
        self.center = c
        self.color = col
        self.reflect = dif  #sets diffuse or reflectivity
        ...
        if iterations > 1:
            ...
            for light in lightSet:
                for sphere in sphereSet:
                    ...
                    count +=1
                    if shadow is None and sphere == self:
                        shade = 1.0
                    elif shadow is None:
                        count -= 1
                    else:
    ...
    def getColor(self, iterations, ray, poi,sphereSet,lightSet,origCol=None):
        ...
        if iterations > 20:
            ...
            shadow = self.getShadow(0, ray, poi, sphereSet, lightSet)
            diffcol = self.getDiffuse(0,poi,sphereSet,lightSet)
            if diffcol is None:
                diffcol = (1,1,1)
            # color = ((shadow[0]*originalCol[0]*(self.reflect-1)+diffcol[0])/self.reflect,
            #          (shadow[1]*originalCol[1]*(self.reflect-1)+diffcol[1])/self.reflect,
            #          (shadow[2]*originalCol[2]*(self.reflect-1)+diffcol[2])/self.reflect)
            color = (shadow[0]*originalCol[0],
                     shadow[1]*originalCol[1],
                     shadow[2]*originalCol[2])
            if color is None:
